...
class InvokerAgentPolicy(AgentPolicy):
    ...
    def _explore(self, tags_by_loc: dict[tuple[int, int], set[int]]) -> Action:
        """Explore with hash-based pseudo-random direction selection."""
        blocked = set(tags_by_loc)
        blocked.discard(self._center)

        # Use step count + agent_id as seed for pseudo-random direction
        hash_val = (self._state.steps_in_phase * 7 + self._agent_id * 13) % len(WANDER_DIRECTIONS)
        for offset in range(len(WANDER_DIRECTIONS)):
            idx = (hash_val + offset) % len(WANDER_DIRECTIONS)
...
class RandomExplorePolicy(MultiAgentPolicy):

...
class InvokerAgentPolicy(AgentPolicy):
    ...
    def _move_toward(self, target: tuple[int, int],
        ...
        if dr == 0 and dc == 0:
            # On target — alternate between noop and vibe cycling
            self._vibe_cycle_idx += 1
            if self._vibe_cycle_idx % 2 == 0:
                return self._action(self._fallback)  # noop
            return self._cycle_vibe()  # vibe action
...
class AlternatePolicy(MultiAgentPolicy):
